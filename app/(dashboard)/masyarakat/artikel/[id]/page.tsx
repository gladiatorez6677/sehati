...
export default function ArtikelDetailPage() {
  ...
  const fetchArticle = async () => {
    try {
      ...
      if (response.ok) {
        ...
        router.push("/masyarakat/artikel")
      }
    } catch (error) {
  ...
  // Process content: if it's plain text (no HTML tags), convert newlines to paragraphs
  const processContent = (content: string) => {
    const hasHtmlTags = /<[a-z][\s\S]*>/i.test(content)
    if (hasHtmlTags) {
      return content
    }
    // Plain text: split by double newlines into paragraphs, single newlines to <br>
    return content
      .split(/\n\s*\n/)
  ...
  return (
    <div className="p-4 lg:p-8 max-w-4xl mx-auto">
      <Button
        variant="ghost"
        onClick={() => router.push("/masyarakat/artikel")}
        className="mb-6"
      >
        ...
        <Card className="mt-8">
          ...
          <CardContent>
            <p className="text-sm text-gray-600">
              Artikel ini hanya untuk tujuan informasi dan edukasi. Selalu
              konsultasikan dengan dokter atau tenaga kesehatan profesional
              untuk diagnosis dan pengobatan kondisi medis Anda.
            </p>
          </CardContent>
        </Card>
      </article>
